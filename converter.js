...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function resetSlidersToInitial() {
        ...
        // Rerun generation with initial settings
    ...
    function buildOptimizedOptions() {
        ...
        // Fine-tuned mapping based on testing (including Starcraft Pylon)
        ...
        // Advanced Color Precision Logic
            ...
            if (url.startsWith('data:')) {
                ...
                dataUrl = await new Promise((resolve, reject) => {
                    ...
                    port.onMessage.addListener((response) => {
                        ...
                        else reject(new Error(response.error || 'Failed to fetch via background script'));
                    });
                });
            } else {
    ...
    // --- Utility & Helper Functions (Layer Management, Downloads, etc.) ---
    // (This section includes functions like displayPalette, mergeSimilarColors, downloadSVG, etc.)
    // Note: The logic for these functions remains largely the same as the previous version,
    // so they are included here for completeness without detailed comments.
    ...
    function displayPalette() {
        ...
        visibleIndices.forEach((index, ord) => {
            ...
            swatch.className = 'w-8 h-8 rounded border-2 border-white ring-1 ring-gray-300 cursor-pointer';
    ...
    elements.downloadTinkercadBtn.addEventListener('click', () => {
        ...
        const imageName = state.originalImageUrl.split(/[\\/]/).pop().replace(/\.[^/.]+$/, '') || 'image';
    ...
    elements.addMergeRuleBtn.addEventListener('click', () => {
        ...
        row.innerHTML = `
            <span>Merge</span>
            <select data-rule-index="${ruleIndex}" data-type="source" class="border rounded-md p-1">${optionsHTML}</select>
            <span>into</span>
            <select data-rule-index="${ruleIndex}" data-type="target" class="border rounded-md p-1">${optionsHTML}</select>
