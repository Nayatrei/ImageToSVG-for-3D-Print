...
test('logo sidebar controls stay isolated from SVG controls', async ({ page }) => {
    ...
    await expect(page.locator('#sidebar-adjust-section #logo-obj-scale')).toBeVisible();
    await expect(page.locator('#sidebar-adjust-section #logo-obj-thickness')).toBeVisible();
    await expect(page.locator('#sidebar-adjust-section #logo-obj-bed')).toBeVisible();
    await expect(page.locator('#sidebar-adjust-section #logo-obj-margin')).toBeVisible();
...
test('pill, badge, and CTA render cleanly in the logo workflow', async ({ page }) => {
    ...
    for (const preset of PRESETS) {
        ...
        await expect(page.locator('#tab-logo .svg-compare-grid')).toHaveScreenshot(`logo-${preset.name}-compare.png`, {
            animations: 'disabled',
            caret: 'hide'
        });
